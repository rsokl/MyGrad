...
def convolve_numpy(input_image, conv_filter, stride, dilation=None):
    ...
    for ind in np.ndindex(out_shape):
        slices = (slice(None),) + tuple(slice(i * s, i * s + w*d, d) for i, w, s, d in zip(ind, conv_shape, stride, dilation))
...
@given(x=hnp.arrays(shape=(4, 5, 7),
       ...
       w=hnp.arrays(shape=(2, 5, 3),
                    dtype=np.float64,
                    elements=st.floats(-10, 10)))
def test_conv_1d_fwd(x, w):
    """ (N=4, C=5, W=7) x (F=2, C=5, Wf=3); stride=1, dilation=1"""
    conf = dict(stride=(1,), dilation=(1,))
    numpy_conv = conv_bank(x, w, **conf)
    mygrad_conv = conv_nd(x, w, **conf).data
