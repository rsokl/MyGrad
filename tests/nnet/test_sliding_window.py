...
@given(data=st.data(),
       x=hnp.arrays(dtype=float, shape=hnp.array_shapes(max_dims=5, min_dims=1)))
def test_sliding_window(data, x):
    ...
    dilation = data.draw(st.one_of(st.none(), st.tuples(*(st.integers(1, s) for s in max_dilation))), label="dilation")
