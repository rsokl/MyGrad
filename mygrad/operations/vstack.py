...
class Vstack(MultiVarOperation):
    def __call__(self, *input_vars):
        for i, dim in enumerate(list(zip_longest(*[var.data[np.newaxis].shape if var.data.ndim is 1 else var.data.shape for var in input_vars]))):
        ...
        self.indices = list(accumulate([1 if var.data.ndim is 1 else var.data.shape[0] for var in input_vars]))
